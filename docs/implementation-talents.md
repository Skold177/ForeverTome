...
Metadata is emitted when its whitelisted values or missing-field markers change. A fresh build references the applicable node observation even when that metadata was recorded earlier. Pause and world-transition boundaries reuse retained metadata; ordinary zone/subzone changes preserve active scans without starting a new build. Clearing the history clears the cache as well. A manual dump always creates a new build/snapshot even if it is unchanged.
