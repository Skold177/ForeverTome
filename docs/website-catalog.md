...
It checks the saved file every two seconds. Each new saved content produces a dated `catalog-<UTC-save-time>-<export-ID>.json` snapshot and replaces `latest.json` atomically. Identical source content is skipped, including after restarting the watcher. A new generator version regenerates unchanged saves without replacing earlier snapshots. The watcher retains older snapshots even if recording history is later cleared inside the game. Snapshots contain the full save, so use observation IDs to avoid duplicate imports; they are not incremental batches. Keep enough disk space for these snapshots.
