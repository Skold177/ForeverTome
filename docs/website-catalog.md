...
Version 0.3.0 preserves all available observations in `history.json` and derives `database.json` with entities, relationships, field coverage and loot-window evidence. Separate category files cover items, spells, talents, quests, creatures, NPCs, monsters, gathering, recipes, maps, professions, currencies and objects. Each uses `format: "forevertome.catalog-category"` with a `category`, cumulative `entries`, supporting `records`, and the relevant `sessions` and `contexts`. Repeated observations are kept once, and conflicting content under the same ID is rejected. Records include related-record dependencies. Resolve entity references by their stable keys. See the [file-by-file coverage guide](database-coverage.md) and [category schema](../schemas/catalog-category-v1.schema.json).

The watcher writes all JSON files with two-space indentation and line breaks. On upgrade it imports available legacy categories, dated snapshots and `latest.json`, even when the recording is unchanged. Original snapshots remain untouched. Previously discarded records cannot be reconstructed; retention metadata makes this boundary explicit.

Each file publishes atomically. An importer reading multiple files must require matching `exportId` and `generatorVersion`, plus matching `historyId` across cumulative files. The export ID identifies the latest processed save and generator; the history ID identifies accumulated evidence. The desktop shows current-save totals separately from retained-history totals. A partial multi-file update is retried on the next check.

Treat generated files as read-only and back up the export folder. The archive is published before projections, allowing failed or deleted projections to recover from retained evidence. Neither backups nor archived records can recover play that was never saved and exported.

`creatures.json` is the canonical creature table. `npcs.json` is a compatibility alias; `monsters.json` selects creatures observed with a hostile or neutral reaction (1 through 4). Both declare `canonicalCategory: "creatures"`. Import each key once. Monster-view membership is not a universal creature classification.

`gathering.json` contains successful, failed and interrupted player herbalism, mining, skinning and fishing cast observations recognized for build 69893. Each occurrence retains its outcome, spell, timestamp and available player position. Start records remain in history and related evidence; readable attempt identities connect outcomes to their starts. These activity positions are not resource-node coordinates, and successful fishing casts do not prove fish were caught. Training spells, skill books, skill-bonus auras and generic Opening casts are excluded. The build-specific allowlist uses inspected client spell tables; new builds need a reviewed rule. Matching historical successful casts also appear in this view.
...
| `catalog` | `items`, `quests`, `npcs`, `spells`, `talents`, `recipes`, `maps`, `professions`, `currencies`, and `objects` when observed |
