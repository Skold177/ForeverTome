"""Project successful gathering casts using the inspected Forever beta client tables."""
...
RULE_ID = "forever-beta-69893-gathering-casts-v1"
...
def gathering_entries(document: dict) -> list[dict]:
    ...
    for row in document["transactions"]:
        if row["type"] != "spell.succeeded" or row["data"].get("actor") != "player":
            continue
        if row["capture"].get("event") != "UNIT_SPELLCAST_SUCCEEDED" or row["evidence"].get("method") != "direct_event":
            continue
